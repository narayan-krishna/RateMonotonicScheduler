...
//integrate deadlines/preemption into this function
int do_work(int count) {
    int product = 1;
    for (int c = 0; c < count; c++) {
        for(int g = 0; g < 5; g++) {
            for(int i = 0 ; i < 10; i ++) {
                //if clock is past the deadline, return 0;
...
timer_ms clock_timer;

RMS() {
    main_m = new mutex;
    clock_timer = timer_ms();
    add_thread_tasks();
